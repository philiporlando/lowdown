...
async def process_aircraft(
    settings: Settings, elevation: ElevationProvider, st: AircraftState
    ...
    flagged; the terrain lookup runs only inside earshot.
    """
...
class LiveService:
    ...
    async def aircraft_in_box(
        ...
        result: list[dict] = []
        for st in states:
            pa = await process_aircraft(s, self._elevation, st)
            if pa is not None:
